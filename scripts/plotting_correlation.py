...
def plot_correlation():
    ...
    print("Nombre de points par catégorie :")
    print(counts)
    ...
    base_cmap = plt.colormaps["Set1"]
    N = len(cats.cat.categories)
    cmap = mcolors.ListedColormap(base_cmap.colors[:N])

    ax.scatter(
        df["concentration_normalized"],
        df["median_normalized"],
        c=codes, cmap=cmap, alpha=0.3, s=15  
    )
    ...
    for cat, g in df.groupby("Categorie"):
        ...
        g_sel = g.sort_values("date").head(3)

        for i, (_, row) in enumerate(g_sel.iterrows()):
            dx, dy = offsets[i % len(offsets)]

            ax.annotate(
                row["date"].strftime("%Y-%m-%d"),
                xy=(row["concentration_normalized"], row["median_normalized"]),
                xytext=(dx, dy),
    ...
    # --- sauvegarde dans le dossier output ---
    plt.savefig(
        "output/categorisation_camera_NDVI.png",
        dpi=300,
        bbox_inches="tight"
    )
    ...
    ax.plot(xs, ndvi_s, "-", lw=2, label="NDVI")
    ax.plot(xs, cam_s,  "--", lw=2, label="Caméra")
    ...
    # --- sauvegarde dans le dossier output ---
    plt.savefig(
        "output/evolution_temporelle_camera_NDVI.png",
        dpi=300,
        bbox_inches="tight"
    )
    ...
    # --- sauvegarde dans le dossier output ---
    plt.savefig(
        "output/cycle_saisonnier_camera_NDVI.png",
        dpi=300,
        bbox_inches="tight"
    )
